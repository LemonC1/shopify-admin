...
class CreateOrder extends Component {
  ...
  handleSubmit = e => {
    ...
    this.setState({ totalprice: 0.0, display: 'block', note: '', loadings: true, customerid: 0 });
    setTimeout(() => {
  ...
  render() {
    ...
    const noimage =
      'https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=33325274,822918042&fm=26&gp=0.jpg';
    console.log('item', customer);

    return (
      <PageHeaderWrapper content={<Link to={{ pathname: '/order/all' }}>返回订单</Link>}>
              ...
              <Card bordered={true}>
                  ...
                  >
                    {list.map((item, index) => (
                      <Option value={JSON.stringify(item)} key={index}>
                        <img
                          style={{ width: '20px' }}
                          src={item.image == null ? noimage : item.image.src}
                        />{' '}
                        {item.title}
                      </Option>
                    ))}
                ...
                <div style={{ display: item == 0 ? 'none' : 'block' }}>
                  <Row>
                    <Col style={{ display: 'flex' }}>
                      <img
                        style={{ width: '70px', flex: 1, display: 'block', marginRight: '10px' }}
                        src={image}
                      />
                      <div style={{ flex: 3 }}>
                      ...
                      >
                        ...
                        >
                          ...
                          <Button
                            size="small"
                            icon="minus"
                            onClick={this.state.count == 1 ? '' : this.changeCount.bind(this, '-')}
                          ></Button>
                        </div>
                ...
                <FormItem>
                  <Button
                    ...
                    onClick={this.changestatus.bind(this, 'paid')}
                  >
                    标记为已付款
                  </Button>
                  <Button type="primary" onClick={this.changestatus.bind(this, 'pending')}>
                    标记为待处理
                  </Button>
                </FormItem>
                {/* 创建顾客 */}
                <div className={styles.cus}>
                  <FormItem label="创建顾客">
                    ...
                    <div style={{ display: this.state.display == 'none' ? 'block' : 'none' }}>
                      <div>
                        <span>{this.state.ordercount} order</span>{' '}
                        <span
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around' }}>
              <FormItem label="First name">
                {getFieldDecorator('first_name')(
                  <Input style={{ width: '200px' }} size="large" onBlur={this.changeFirst} />,
                )}
              </FormItem>
              <FormItem label="Last name">
                {getFieldDecorator('last_name')(
                  <Input style={{ width: '200px' }} size="large" onBlur={this.changeLast} />,
                )}
              </FormItem>
            </div>
